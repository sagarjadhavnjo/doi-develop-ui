...
@Component({
  selector: 'app-claim-listing-send-for-payment',
  templateUrl: './claim-listing-send-for-payment.component.html',
  styleUrls: ['./claim-listing-send-for-payment.component.css']
})
export class ClaimListingSendForPaymentComponent implements OnInit {

  todayDate = new Date();
  claimListingSendForPaymentForm: FormGroup;
  ...
  constructor(private fb: FormBuilder, private router: Router, private workflowService: DoiService) { }

  ngOnInit() {
    this.claimListingSendForPaymentForm = this.fb.group({
      claimId: [''],
      policyNo: [''],
      employeeName: [''],
      district: [''],
  ...
  selectDistrict() {
    const district = this.claimListingSendForPaymentForm.value.district;
