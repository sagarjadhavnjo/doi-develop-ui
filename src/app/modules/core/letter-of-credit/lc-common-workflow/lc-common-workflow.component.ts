...
} from '../lc-common-workflow-constant/lc-common-workflow.constants';
...
@Component({
    selector: 'app-lc-common-workflow',
    templateUrl: './lc-common-workflow.component.html',
    styleUrls: ['./lc-common-workflow.component.css']
})
export class LcCommonWorkflowComponent implements OnInit {
        ...
        try {
            if (selectedAction.locationTypeName) {
                if (selectedAction.locationTypeName === DataConstant.CUSTOM_LOCATION_TYPE) {
                    if (this.data.menuModuleName === ModuleNames.LOC) {
...
onHodChange() {
    try {
        this.userList = [];
        this.userResponse = [];
        if (this.workflowForm.get('hod').value) {
            if (this.selectedAction.isToUserSelReq === 1 || this.selectedAction.isToUserSelReq === -1) {
                this.isUserVisible = true;
                this.workflowForm.addControl('user', new FormControl(Validators.required));
                this.workflowForm.controls.user.setValue('');
                this.getWorkflowUsers(this.workflowForm.get('hod').value);
            }
        }
    } catch (err) {
        this.toastr.error(err);
...
onCoChange() {
    try {
        this.userList = [];
        this.userResponse = [];
        if (this.workflowForm.get('co').value) {
            if (this.selectedAction.isToUserSelReq === 1 || this.selectedAction.isToUserSelReq === -1) {
                this.isUserVisible = true;
                this.workflowForm.addControl('user', new FormControl(Validators.required));
                this.workflowForm.controls.user.setValue('');
                this.getWorkflowUsers(this.workflowForm.get('co').value);
            }
        }
    } catch (err) {
        this.toastr.error(err);
