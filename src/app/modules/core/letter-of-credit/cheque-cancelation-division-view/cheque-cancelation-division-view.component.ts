    ...
    this.locService.getData(params, APIConst.LC_CHEQUE_CANCEL.EDITVIEW_CHEQUECANCEL).subscribe(
      (res: any) => {
        ...
        if (res && res.result && res.status === 200) {
          ...
          this.chequeCancellationForm.patchValue({ chequeDate: (res.result.chequeDate) ? this.datepipe.transform(res.result.chequeDate, 'dd-MMM-yyyy')
          ...
          this.refDate = res.result.refNoDate;
