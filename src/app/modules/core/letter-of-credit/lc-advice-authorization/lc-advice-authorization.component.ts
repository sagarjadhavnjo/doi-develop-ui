...
export class LcAdviceAuthorizationComponent implements OnInit {
    ...
    LcAdviceVerificationDATAColumn: string[] = [
        ...
        'ddoApproverName', 'adviceGrossAmount', 'totalDeduction', 'adviceNetAmount','paymentType',
        'receivedFrom', 'receivedDate', 'sentTo', 'sentDate', 'lyingWith', 'workflowStatus', 'status', 'authorizedReturnedDate',
        'action'//DptName	     PaymentType	workFlwStatus 
 ...
 */
   ...
   */
    fillDropdownOfFilter() {
        this.locService.getDataWithoutParamsPost(APIConst.LC_ADVICEPREPARATION.ADVICE_AUTHORIZATION_DROP_DOWN).subscribe(
            (resp: any) => {
    ...
    edit(element) {
        this.router.navigate(
            ['/dashboard/lc/lc-advice-preparation-view', {
                ...
                isEditable: element.isEditable, listingUrl: '/dashboard/lc/lc-advice-authorization'
                , headingName: 'LC Advice Authorization'
            }],
    ...
    getListData(event: any = null, isReset: boolean = false) {
        ...
        if (
            ...
            this.locService.getData(passData, APIConst.LC_ADVICEPREPARATION.GET_ADVICE_AUTHORIZATION_LISTING).subscribe(
                (res: any) => {
  ...
  *
  *
  * @memberOf LcAdviceAuthorizationComponent
  */
