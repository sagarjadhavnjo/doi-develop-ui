...
export class IncrementListComponent implements OnInit {
    ...
    incrementDisplayedColumns = [
        'id', 'tranNo', 'incrementFor', 'empPayType', 'tranStatus', 'wfStatus', 'action'
    ];
     ...
     */
    getSearchFilter() {
        const obj = this.incrementSearchForm.value;
        if (
            !obj.fromDate &&
            !obj.toDate &&
            !obj.transNo &&
        ...
        for (const key in obj) {
            const currObj = {};
            currObj['key'] = key;
            if (key === 'transNo' || key === 'fromDate' || key === 'toDate' || key === 'wfStatus') {
            ...
            } else if (key === 'empTypeId' ||  key === 'empPayTypeId' ||
                key === 'classId' || key === '  ' ||
                key === 'incrementFor' || key === 'officeId' || key === 'postId' ||
