...
import { SENIOR_SCALE_EVENT_ERRORS, SENIOR_SCALE_APIS } from './../index';
...
@Component({
    selector: 'app-employee-current-details',
    templateUrl: './employee-current-details.component.html',
    styleUrls: ['./employee-current-details.component.css']
})
export class EmployeeCurrentDetailsComponent implements OnInit, OnDestroy, OnChanges {
    ...
    @Input() clearExamDetails = false;
    @Input() pvuScreen: boolean;
    ...
    setEmployeeInfo() {
        if (this.employeeDetails && this.employeeDetails.employee) {
            ...
            if (this.employeeDetails.employee.departmentCategoryId === 13
                || this.employeeDetails.employee.departmentCategoryId === 17) {
    ...
    ngOnChanges() {
        ...
        if (!this.clearEmployeeDetails && this.employeeCurrentForm
            && this.eventDetails && this.eventDetails.employeeNo) {
            ...
            if (this.eventDetails && (this.eventDetails.statusId === 205 || this.eventDetails.statusId === 0)) {
                ...
                employee['payScale'] = this.eventDetails.cPayScaleId;
    ...
    getEmployeeDetail() {
        if (!this.eventDetails ||
            (this.eventDetails && (this.eventDetails.statusId === 205 || this.eventDetails.statusId === 0))) {
            const self = this,
                dataToSend = {},
                effectiveDate: Date = this.eventForm.controls.eventEffectiveDate.value;
            dataToSend['pageIndex'] = 0;
            dataToSend['pageElement'] = 10;
            dataToSend['jsonArr'] = [
                {
                    'key': 'empNo',
            ...
            this.eventService.getEmployeeDetails(SENIOR_SCALE_APIS.EMPLOYEE_DETAILS, dataToSend).subscribe(res => {
    ...
    /**
     * To get the exam details
     */
    getExamDetails() {
        const self = this;
        this.eventService.getEmployeeExamDetails({
            request: {
                id: this.eventDetails.employeeId,
                casEvent: 0
            }
        }).subscribe(res => {
            if (res['status'] === 200 && res['result']) {
